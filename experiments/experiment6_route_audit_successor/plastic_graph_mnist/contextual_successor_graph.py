...
class ContextualSuccessorGraphConfig:
    """Sparse recurrent graph for Experiment 6's route-audited traversal world."""
...
class ContextualTraversalResult:
    ...
    step_concept_overlaps: np.ndarray
    step_mode_overlaps: np.ndarray
...
class ContextualSparseRecurrentGraph:
    """Context-conditioned recurrent plastic graph with audited multi-step traversal."""
    ...
    def traverse(self, start_concept: int, mode: str, steps: int) -> ContextualTraversalResult:
        ...
        total_drive = 0.0
        step_concept_overlaps: list[np.ndarray] = []
        step_mode_overlaps: list[np.ndarray] = []
        for _ in range(steps):
            active, drive_norm, _drive = self.step(active, mode)
            total_drive += drive_norm
            seen.update(int(x) for x in active)
            _predicted, _confidence, concept_overlaps, _predicted_mode, _context_confidence, mode_overlaps = self.decode_state(active)
            step_concept_overlaps.append(concept_overlaps.copy())
            step_mode_overlaps.append(mode_overlaps.copy())
        ...
        return ContextualTraversalResult(
            ...
            step_concept_overlaps=np.stack(step_concept_overlaps, axis=0) if step_concept_overlaps else np.zeros((0, self.config.num_concepts), dtype=np.float32),
            step_mode_overlaps=np.stack(step_mode_overlaps, axis=0) if step_mode_overlaps else np.zeros((0, len(self.config.mode_names)), dtype=np.float32),
        )
    ...
    def _apply_update(self, source_concept: int, mode: str, target_concept: int, reward: float) -> tuple[int, int, int]:
        ...
        for source in source_active:
            ...
            if self.config.use_structural_plasticity:
                ...
                if reward < 0.0 and bad_mask.any():
                    candidate_slots = np.flatnonzero(bad_mask)
                    if len(candidate_slots) > replace_count:
                        order = np.argsort(weights[candidate_slots])[:replace_count]
                        chosen = candidate_slots[order]
                    else:
                        weakest = np.argpartition(weights, replace_count - 1)[:replace_count]
                        combined = np.unique(np.concatenate([candidate_slots, weakest]))
                        order = np.argsort(weights[combined])[:replace_count]
                        chosen = combined[order]
                    rewrite_bonus = self.config.learning_rate * max(abs(reward), 0.5)
                else:
                    chosen = np.argpartition(weights, replace_count - 1)[:replace_count]
                    rewrite_bonus = self.config.learning_rate * max(reward, 0.25)
                chosen = np.asarray(chosen, dtype=np.int32)
                if len(chosen) > 0:
                    new_targets = self.rng.choice(target_active, size=len(chosen), replace=True)
                    targets[chosen] = new_targets.astype(np.int32)
                    weights[chosen] = np.maximum(weights[chosen], 0.0) + rewrite_bonus
                    replacements += int(len(chosen))
