...
& $pythonExe (Join-Path $scriptDir "run_exp5_suite.py") `
    --max-number 24 `
    --max-steps 5 `
...
& $pythonExe (Join-Path $scriptDir "analyze_exp5_suite.py") `
    --db (Join-Path $scriptDir "runs\exp5_contextual_successor_suite.sqlite3") `
    --output-dir (Join-Path $scriptDir "analysis\exp5")
