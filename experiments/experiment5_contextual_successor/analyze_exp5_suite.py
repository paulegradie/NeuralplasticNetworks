...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze Experiment 5 contextual successor suite.")
    parser.add_argument("--db", type=Path, default=Path("runs/exp5_contextual_successor_suite.sqlite3"))
    parser.add_argument("--output-dir", type=Path, default=Path("analysis/exp5"))
...
def plot_accuracy_by_mode(summary: pd.DataFrame, output: Path) -> None:
    ...
    plt.ylim(0.0, 1.05)
    plt.ylabel("accuracy")
    plt.title("Experiment 5: composition accuracy by mode")
    plt.legend()
    plt.tight_layout()
    plt.savefig(output, dpi=150)
    plt.close()
...
def plot_accuracy_by_path_length(summary: pd.DataFrame, output: Path) -> None:
    ...
    plt.xlabel("steps")
    plt.ylabel("accuracy")
    plt.title("Experiment 5: composition accuracy by path length")
    plt.legend()
    plt.tight_layout()
    plt.savefig(output, dpi=150)
    plt.close()
...
def plot_confusion(summary: pd.DataFrame, run_name: str, output: Path) -> None:
    ...
    plt.title(f"Experiment 5: context confusion ({row.iloc[0]['run_name']})")
...
def plot_adaptation_curve(metrics: pd.DataFrame, runs: pd.DataFrame, output: Path) -> None:
    ...
    plt.title("Experiment 5: composition accuracy over training")
    plt.legend()
    plt.tight_layout()
    plt.savefig(output, dpi=150)
    plt.close()
...
def main() -> None:
    ...
    summary.to_csv(args.output_dir / "exp5_comparison.csv", index=False)

    plot_bar(summary, "run_name", "best_composition_accuracy", "Experiment 5: best composition accuracy", args.output_dir / "exp5_best_composition_accuracy.png")
    if "composition/average_recurrent_drive" in summary.columns:
        plot_bar(summary, "run_name", "composition/average_recurrent_drive", "Experiment 5: latest recurrent drive", args.output_dir / "exp5_recurrent_drive.png")
    if "composition/average_wrong_route_activation" in summary.columns:
        plot_bar(summary, "run_name", "composition/average_wrong_route_activation", "Experiment 5: wrong-route activation", args.output_dir / "exp5_wrong_route_activation.png")
    plot_accuracy_by_mode(summary, args.output_dir / "exp5_accuracy_by_mode.png")
    plot_accuracy_by_path_length(summary, args.output_dir / "exp5_accuracy_by_path_length.png")
    plot_confusion(summary, "exp5_full_contextual_traversal", args.output_dir / "exp5_context_confusion.png")
    plot_adaptation_curve(metrics, runs, args.output_dir / "exp5_adaptation_curve.png")
    ...
    (args.output_dir / "exp5_report.md").write_text("\n".join(report), encoding="utf-8")
