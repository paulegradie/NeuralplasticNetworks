"""Experiment 9 core: robust adaptive route plasticity.

Experiment 8 showed that a local plastic graph can acquire a clean context-conditioned
route field from one-step transition experience and compose unseen multi-step paths.
Experiment 9 stresses that mechanism: context interference should make inhibition
useful, while noisy or delayed feedback should make reward gating and eligibility
traces useful.

The implementation remains intentionally inspectable. It models a symbolic number
world using sparse distributed number, mode, and source-mode pair assemblies over a
latent hidden population. A local Hebbian-style plastic route matrix is learned only
from one-step transition experiences. Multi-step composition is evaluated by repeated
recurrent traversal of the learned one-step route field.
"""
...
class AssemblyWorld:
    ...
    def _maybe_dropout(self, arr: np.ndarray, dropout: float, rng: np.random.Generator) -> np.ndarray:
        if dropout <= 0.0 or arr.size == 0:
            return arr
        keep = max(1, int(round(arr.size * (1.0 - dropout))))
        keep = min(keep, arr.size)
        return rng.choice(arr, size=keep, replace=False)
    ...
    def combined_pre(self, source: int, mode: str, variant: VariantConfig, rng: np.random.Generator, include_context: bool = True) -> np.ndarray:
        ...
        if include_context and variant.context_binding:
            mode_part = self._maybe_dropout(self.mode(mode), variant.context_dropout, rng)
            pair_part = self._maybe_dropout(self.pair(source, mode), variant.context_dropout, rng)
            parts.append(mode_part)
            parts.append(pair_part)
...
class RobustRouteGraph:
    """Local-plastic recurrent route graph with interference/noise/delay stressors."""
    ...
    def finalize(self) -> None:
        ...
        # Use a separate deterministic generator for evaluation/cache construction so
        # dropout/bleed are fixed per finalized run and not a function of call ordering.
        cache_rng = np.random.default_rng(self.seed + 987654)
        for source in range(self.max_number + 1):
            for mode in MODE_NAMES:
                pre = self.world.combined_pre(source, mode, self.variant, cache_rng, include_context=True)
    ...
    def _apply_update(self, source: int, mode: str, observed_target: int, allow_reward_gate: bool = True) -> None:
        v = self.variant
        self._score_cache = None
        if not v.structural_plasticity:
            return
        if allow_reward_gate and v.reward_gate and observed_target < 0:
            return
    ...
    def _feedback_target(self, true_target_n: int) -> int:
        ...
        if v.feedback_noise > 0.0 and self.rng.random() < v.feedback_noise:
            ...
            # Reward-gated variant detects the unreliable signal and skips it.
            # No-reward-gate variant commits the misleading update.
    ...
    def train_transition(self, source: int, mode: str, true_target_n: int) -> None:
        self._apply_update(source, mode, self._feedback_target(true_target_n))

    def train_curriculum(self, transition_tasks: Sequence[Task], repeats: int) -> None:
        ...
        # Delayed feedback: after delay steps, the target label for an earlier
        # transition arrives. With an eligibility trace we update the original
        # source/mode association. Without it, we incorrectly update the current
        # active source/mode with the delayed target, causing credit assignment errors.
        pending: List[Task] = []
        for task in stream:
            pending.append(task)
            if len(pending) > delay:
                ...
                if self.variant.eligibility_trace:
                    self._apply_update(original.start, original.mode, observed)
                else:
                    self._apply_update(task.start, task.mode, observed)
        # Flush remaining delayed events. No-eligibility variant now has no reliable
        # current state, so it uses the last active state, again producing ambiguity.
        ...
        while pending:
            ...
            if self.variant.eligibility_trace:
                ...
                self._apply_update(last.start, last.mode, observed)
    ...
    def predict(self, task: Task) -> PredictionTrace:
        ...
        for step_i in range(steps_to_run):
            ...
            local_expected = expected_next if current == expected_current else true_target(current, task.mode, 1)
            if not (0 <= local_expected <= self.max_number):
                local_expected = predicted_next
            rank, margin, _, _ = self.target_rank_and_margin(current, task.mode, local_expected)
            ranks.append(rank)
            margins.append(margin)
            context_margins.append(self.context_margin(current, task.mode, local_expected))
            wrong_acts.append(self.wrong_route_activation(current, task.mode, local_expected))
...
def make_interference_variants(context_bleed: float) -> List[VariantConfig]:
    return [
        VariantConfig(name="exp9_full_interference_robust", context_bleed=context_bleed),
        VariantConfig(name="exp9_no_inhibition", inhibition=False, context_bleed=context_bleed),
        VariantConfig(name="exp9_no_context_binding", context_binding=False, context_bleed=context_bleed),
        VariantConfig(name="exp9_no_structural_plasticity", structural_plasticity=False, context_bleed=context_bleed),
    ]


def make_feedback_variants(feedback_noise: float, reward_delay_steps: int) -> List[VariantConfig]:
    return [
        VariantConfig(name="exp9_full_reward_robust", feedback_noise=feedback_noise, reward_delay_steps=reward_delay_steps),
        VariantConfig(name="exp9_no_reward_gate", reward_gate=False, feedback_noise=feedback_noise, reward_delay_steps=reward_delay_steps),
        VariantConfig(name="exp9_no_eligibility_trace", eligibility_trace=False, feedback_noise=feedback_noise, reward_delay_steps=reward_delay_steps),
        VariantConfig(name="exp9_no_structural_plasticity", structural_plasticity=False, feedback_noise=feedback_noise, reward_delay_steps=reward_delay_steps),
        VariantConfig(name="exp9_no_recurrence", recurrence=False, feedback_noise=feedback_noise, reward_delay_steps=reward_delay_steps),
    ]
