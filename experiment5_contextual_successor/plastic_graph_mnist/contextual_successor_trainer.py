...
class ContextualSuccessorExperimentTrainer:
    """Trains and evaluates Experiment 5's contextual traversal world."""
    ...
    def start(self) -> int:
        config = {
            "experiment_phase": "exp5_contextual_successor_world",
    ...
    def evaluate_examples(self, examples: Iterable[ContextualSequenceExample]) -> Dict[str, float]:
        ...
        for example in examples:
            ...
            for other_mode in self.task.mode_names:
                ...
                if other_path is None:
                    continue
                competing_overlap = max(competing_overlap, float(result.concept_overlaps[other_path[-1]]))
            wrong_routes.append(competing_overlap)
            margins.append(float(result.concept_overlaps[example.target]) - competing_overlap)
        ...
        metrics: Dict[str, float] = {
            ...
            "average_context_margin": float(np.mean(margins)),
