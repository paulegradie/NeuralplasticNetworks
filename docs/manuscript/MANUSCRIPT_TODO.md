...
Use the completed Exp13.2 baseline-suite run to update manuscript framing, then audit lesion diagnostics and decide whether additional neural baselines or latent-context evidence are required.

Claim: Exp13.2 partially resolves the baseline blocker with symbolic/algorithmic baselines, but does not make the manuscript submission-ready.
Evidence: `experiments/experiment13_2_baseline_suite/analysis/exp13_2_full_20260507_165813/validation_report.md` reports PASS 28, WARN 0, FAIL 0, and the digest identifies the oracle context-gated match to CIRM plus baseline failure modes.
Caveat: Baselines are symbolic/algorithmic, not full neural baselines; targeted lesion evidence from Exp13.1 still failed the expected pattern; prior-art import and final figure scripts remain open.
Source path: `docs/threads/experiment13_2_analysis_digest.md`; `docs/experiments/exp13_2_summary.md`; `experiments/experiment13_2_baseline_suite/README.md`; `experiments/experiment13_2_baseline_suite/analysis/exp13_2_full_20260507_165813/validation_report.md`

## P0 - Required Before Manuscript Submission

| TODO | Reason | Related experiment | Source thread | Target output |
|---|---|---|---|---|
| Decide whether additional neural baselines are required beyond Exp13.2. | Exp13.2 partially resolves C12 with symbolic/algorithmic baselines, but reviewers may still require neural or literature-specific comparators. | Exp13.2 and manuscript-level | `docs/threads/experiment13_2_analysis_digest.md`; `docs/manuscript/BASELINE_REQUIREMENTS.md`; `docs/manuscript/CLAIMS_AND_EVIDENCE.md` | Baseline-results framing in the manuscript plus any added neural/prior-art baseline plan. |
...
| Decide placement for Exp13.2 baseline figure/table. | The oracle context-gated table matching CIRM is manuscript-important claim refinement. | Exp13.2 | `docs/threads/experiment13_2_analysis_digest.md`; `docs/manuscript/FIGURE_PLAN.md` | Main or supplementary baseline panel/table generated from reproducible scripts. |
...
| Design latent world inference. | Exp13.2 shows oracle context-gated lookup solves the clean supplied-context benchmark, leaving latent context inference as a conceptual limitation. | Future Exp14 | `docs/threads/experiment13_2_analysis_digest.md`; `docs/threads/experiment12to13_export.md` | Experiment 14 design doc after P0/P1 scientific hardening if needed before submission. |
...
| Exp13.2 baseline-suite run imported. | Full run artifacts were validated and imported into thread index, experiment summary, README completed-run log, claims, figure plan, limitations, TODOs, synthesis docs, source-data mirrors, artifact indexes, and import report. | `docs/threads/experiment13_2_analysis_digest.md`; `docs/experiments/exp13_2_summary.md`; `docs/repo_audit/EXP13_2_ANALYSIS_IMPORT_REPORT.md` |
