...
| `docs/threads/experiment13_2_analysis_digest.md` | Exp13.2 | Analyze completed baseline-suite full run and refine manuscript claims around symbolic baselines. | yes, for Exp13.2 full run | yes, future Exp14 latent-context inference recommendation only | yes, Exp13.2 summary, README, registry, synthesis docs | yes, as conservative baseline and claim-refinement updates | SQLite database was absent from the uploaded digest bundle but is present locally at `experiments/experiment13_2_baseline_suite/runs/exp13_2_full_20260507_165813.sqlite3`; oracle context-gated lookup matches CIRM on the clean supplied-context benchmark. |
...
- Exp11-Exp13.2 provide the current candidate main-result spine. Exp13.1 adds completed publication-hardening evidence for recurrence, structural plasticity, context binding, local-budget damage, and freeze-plasticity diagnostics, while marking lesion caveats. Exp13.2 partially resolves the baseline blocker with symbolic/algorithmic baselines and refines the clean supplied-context claim because oracle context-gated lookup matches CIRM.
